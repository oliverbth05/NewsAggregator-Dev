import React, { Component } from 'react';
import {Link} from 'react-router-dom';
import axios from 'axios';
import { connect } from 'react-redux';
import { loadSourcesAsync } from '../store/actions';

import './sources.css';
class Sources extends Component {
    
    state = {
        filtered: []
    }
    
    componentDidMount() {
        
        if (this.props.sources.length === 0) {
            this.props.loadSources();
        }
    }
    
    searchSourceHandler(e) {
        var arr = this.props.sources.filter((item) => {
            if (item.name.slice(0, e.target.value.length).toLowerCase() === e.target.value.toLowerCase()) {
                return true
            }
        })
        
        console.log(arr)
        
        this.setState({
            filtered: arr
        })
    }
    
    
    
    render() {
        
        var filteredMapped;
        
        if(this.state.filtered.length > 0) {
            filteredMapped = this.state.filtered.map(item => {
                return (
                    <li className = 'filter-item'>
                    <Link className = 'filter-link' to = {'/sources/' + item.id}>
                    {item.name}
                    </Link>
                    </li>
                    )
            })
        }
        
        return(
            <div className = 'page-container'>
                <h1 className = 'page-heading' >Browse the latest articles from a number of different news sources</h1>
                
                
                <div className = 'source-menu'>
                
                    <h2 className = 'sub-heading' >Popular News Outlets</h2>
                    
                    <div className = 'source-grid'>
                        <Link to = '/sources/abc-news' className ='source-grid-element'>
                            <img src = 'http://youthh2o.com/wp-content/uploads/2014/02/abc-news-thumbnail.png' />
                            <p>ABC News</p>
                        </Link>
                        
                        <Link  to = '/sources/axios' className ='source-grid-element'>
                            <img src = 'https://i.vimeocdn.com/portrait/25764922_300x300' />
                            <p>Axios</p>
                        </Link>
                        
                        <Link to = '/sources/bbc-news' className ='source-grid-element'>
                            <img src = 'https://images-na.ssl-images-amazon.com/images/I/61C6z7hAXEL._SL300_.png' />
                            <p>BBC News</p>
                        </Link>
                        
                         <Link to = '/sources/cnn' className ='source-grid-element'>
                            <img src = 'https://i.vimeocdn.com/portrait/6921642_300x300' />
                            <p>CNN</p>
                        </Link>
                        
                        <Link to = '/sources/espn' className ='source-grid-element'>
                            <img src = 'http://www.vault.com/media/9733914/espn-logo.jpg' />
                            <p>ESPN</p>
                        </Link>
                        
                        <Link to = '/sources/fox-news' className ='source-grid-element'>
                            <img src = 'http://myamazingtv.com/wp-content/uploads/2017/10/Get-Fox-News-on-an-Android-TV-Box.png' />
                            <p>Fox News</p>
                        </Link>
                        
                        <Link to = '/sources/independent' className ='source-grid-element'>
                            <img src = 'http://store-images.s-microsoft.com/image/apps.60444.13510798886637349.59fc6d31-c1fe-44d4-b856-99bc2226bc0a.1c554776-aa9c-44e8-8e85-0f47312e71c5' />
                            <p>Independent</p>
                        </Link>
                        
                        <Link to = '/sources/polygon' className ='source-grid-element'>
                            <img src = 'https://lh3.googleusercontent.com/smwVr7L1PC-grQWbdvPeMAhAmFyOCQck_pMNMYUQIYDHrfkx9ZXnUJ_18OcrHxXmprT8-Kqe=rj-c-w300-h300-l95-c0xffffff' />
                            <p>Polygon</p>
                        </Link>
                        
                        <Link to = '/sources/techradar' className ='source-grid-element'>
                            <img src = 'https://media.licdn.com/dms/image/C4D0BAQGYAujoOo1yDQ/company-logo_200_200/0?e=2159024400&v=beta&t=5gNY8aFdOxu4RlX8BWFkHknna0TWsmfk-WpFbPLbe9E' />
                            <p>Tech Radar</p>
                        </Link>
                        
                        <Link to = '/sources/the-verge' className ='source-grid-element'>
                            <img src = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQoWTRwo0GkcxM1pQ3ohQdaSAW2rMgUGyxeTmjAsSKm50EfgRbsGg' />
                            <p>The Verge</p>
                        </Link>
                        
                        <Link to = '/sources/time' className ='source-grid-element'>
                            <img src = 'data:image/png;base64,' />
                            <p>Time</p>
                        </Link>
                        
                        <Link to = '/sources/the-washington-post' className ='source-grid-element'>
                            <img src = 'data:image/jpeg;base64,' />
                            <p>The Washington Post</p>
                        </Link>
                    </div>
                    
                    <div className = 'source-search'>
                        <h2 className = 'sub-heading' >Find a Source</h2>
                        <input type = 'text' onChange = {this.searchSourceHandler.bind(this)} />
                    </div>
                    <h3 className = 'sub-heading'> {this.state.filtered ? this.state.filtered.length + ' result(s)' : null}</h3>
                    <ul className = 'source-filter'>
                    
                        {filteredMapped}
                        
                    </ul>
                    
                   
                </div>
            </div>
        )
    }
}

const mapStateToProps = (state) => {
    return {
        sources: state.sources
    }
}

const mapDispatchToProps = (dispatch) => {
  return {
    loadSources: () => { dispatch(loadSourcesAsync()) }
  }
}

export default connect(mapStateToProps, mapDispatchToProps)(Sources);